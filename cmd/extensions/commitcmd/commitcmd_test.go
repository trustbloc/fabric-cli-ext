...
func TestCommitCmd(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "1", "--peer", "peer1")
		require.NoError(t, c.Execute())
		require.Contains(t, w.Written(), msgCCCommitted)
	})

	t.Run("With policy -> Success", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "1", "--peer", "peer1", "--policy", "OR('Org1.member','Org2.member')")
		require.NoError(t, c.Execute())
		require.Contains(t, w.Written(), msgCCCommitted, w.Written())
	})

	t.Run("With invalid policy -> Success", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "1", "--peer", "peer1", "--policy", "OR('Org1.member','Org2')")
	...
	t.Run("With collections config -> Success", func(t *testing.T) {
		...
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "1", "--peer", "peer1", "--collections-config", collsCfg)
		require.NoError(t, c.Execute())
		require.Contains(t, w.Written(), msgCCCommitted)
	})
