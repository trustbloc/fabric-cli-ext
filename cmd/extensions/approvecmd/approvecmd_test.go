...
func TestApproveCmd(t *testing.T) {
	...
	t.Run("Missing package ID arg", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1")
		require.EqualError(t, c.Execute(), "package ID not specified")
		require.Equal(t, "Error: package ID not specified", w.Written())
	})

	t.Run("Missing sequence arg", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "cc1:v1")
		require.EqualError(t, c.Execute(), "sequence not specified")
		require.Equal(t, "Error: sequence not specified", w.Written())
	})

	t.Run("Invalid sequence arg", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "cc1:v1", "xxx")
		err := c.Execute()
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid sequence")
		require.Contains(t, w.Written(), "Error: invalid sequence")
	})

	t.Run("Invalid sequence arg", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "cc1:v1", "0")
		require.EqualError(t, c.Execute(), "sequence must be greater than 0")
		require.Equal(t, "Error: sequence must be greater than 0", w.Written())
	})

	t.Run("Success", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "cc1:v1", "1")
		require.NoError(t, c.Execute())
		require.Contains(t, w.Written(), msgCCApproved)
	})

	t.Run("With policy -> Success", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "cc1:v1", "1", "--policy", "OR('Org1.member','Org2.member')")
		require.NoError(t, c.Execute())
		require.Contains(t, w.Written(), msgCCApproved, w.Written())
	})

	t.Run("With invalid policy -> Success", func(t *testing.T) {
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "cc1:v1", "1", "--policy", "OR('Org1.member','Org2')")
	...
	t.Run("With collections config -> Success", func(t *testing.T) {
		...
		w := &mocks.Writer{}
		c := newMockCmd(t, w, p, "cc1", "v1", "cc1:v1", "1", "--collections-config", collsCfg)
		require.NoError(t, c.Execute())
		require.Contains(t, w.Written(), msgCCApproved)
	})
